...
| `/apply/[productId]` | Dynamic steps: Party (1–6 travelers) → Details (if fields, per traveler) → Documents (if product docs, per traveler) → Review → Payment (fees × N). Passport scan + passport fields only when `passport_scan` is on the product. **Upload from Mobile** (QR → phone capture → WebSocket/poll sync) is available for **single traveler only**; hidden when party length > 1. On Documents, the QR CTA sits above the checklist and stays sticky under the site header; step changes scroll to the step heading so the CTA is in view. Required docs are listed first (optional collapsed); progress counts required uploads only. Mobile uses a unified sticky footer (fee + Back/Save/Continue). Completed stepper steps are clickable to jump back; Review offers Edit links per section. APIs: `POST /api/cases` (`travelers[]`), `PATCH /api/cases/drafts/{id}` (customer JWT **or** `session_id` for phone), traveler add/remove, `POST /api/documents/upload` (`session_id` optional), `POST /api/documents/notify-upload`, `GET/POST /api/documents/session/{id}`, `WS /api/documents/ws/{id}`, `POST /api/documents/scan-passport`, checkout → N linked cases + `case_group_id` |
